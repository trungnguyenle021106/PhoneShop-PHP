<?php
require('JS/Database.php');
$connect = new MyConnection("127.0.0.1", "root", "", "phoneshops");
$connect->connectDB();
?>
    ...
    <table>
        ...
        <?php
                foreach( $connect->read("san_pham") as $row){
                    ?>
                <tr>
                 <td id="SP_MASP"><?php echo $row['MA_SP']; ?></td>
                 <td id="SP_MALOAI"><?php echo $row['MA_LOAI']; ?></td>
                 <td id="SP_MANSX"><?php echo $row['MA_NSX']; ?></td>
                 <td id="SP_TEN"><?php echo $row['TEN_SP']; ?></td>
                 <td id="SP_GIA"><?php echo $row['GIA_BAN']; ?></td>
                 <td id="SP_ANH"><img src="image_admin/<?php echo $row['HINH_ANH']; ?>" alt="##" style="height: 50px;"></td>
                 <td id="SP_SL"><?php echo $row['SO_LUONG']; ?></td>     
                 <form action="" method="POST" id="dieukien_xoa">
                    <input type="hidden" name="page" value="<?php echo $_POST['page']; ?>">
                    <input type="hidden" name="MASP_xoa" value="<?php echo $row['MA_SP']; ?>">

                    <td><input type="submit" name="SP_xoa_btn" id="SP_xoa_btn" class="thaotac" value="xóa"></td>
                </form>            
                    <form action="" method="POST" id="dieukien_sua">
                    <td><input type="button" id="SP_sua_btn" class="thaotac" value="sửa"></td>
                    </form>
                </tr>
                <?php } ?>
        </tbody>
...
<form action="" method="POST" id="form_timkiem_SP">
    ...
    <div>
        ...
        <input type="text" name="ANH_SP" id="ANH_SP_add" placeholder="File ảnh (img.đuôi)"> <span style="color: red; ">(*)</span>
</div>
      <div>
      ...
      <div>
        ...
        <input type="hidden" name="page" value="<?php echo $_POST['page']; ?>">
        <input type="button" class="btn_themSP" name="btn_themSP" value="Nhập cấu hình">
    </form>
    ...
    <div>
        <label for="">Hình ảnh: </label>
        <input type="text" name="AnhSP" id="AnhSP" placeholder="File ảnh"> 
</div>
        <input type="hidden" name="page" value="<?php echo $_POST['page']; ?>">
        <input type="hidden" name="MASP" value="" id="MASP_sua"> 
        <input type="submit" class="btn_suaSP" name="btn_suaSP" value="sửa">
...
<?php
require('JS/form_themCHSP_admin.php');
require('JS/chucnang_SP_admin.php');
?>
