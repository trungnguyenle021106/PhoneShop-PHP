...
// Mảng toàn cục chứa phụ kiện
var dataArray = [];
//

read()

// Code xử lý đọc dữ liệu từ database
function getConditionLoai() {
    ...
    if (selectedLoaiValue !== "") {
        condition = " AND MA_LOAI = " + selectedLoaiValue;
    }
    return condition;
}
...
function getConditionBrand() {
    ...
    if (selectedNSXValue !== "") {
        condition = " AND MA_NSX =" + selectedNSXValue;
    }
    return condition;
}
...
function getConditionInPrice() {
    ...
    condition = " AND GIA_BAN >= " + priceLeftValue + " AND GIA_BAN <= " + priceRightValue;
    return condition;
}
